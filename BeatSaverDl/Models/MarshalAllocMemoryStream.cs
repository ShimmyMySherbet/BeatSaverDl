...
// Copied from my 'NewSocket' project
//https://github.com/ShimmyMySherbet/NewSocket
namespace BeatSaverDl.Models
{
    ...
    public class MarshalAllocMemoryStream : UnmanagedMemoryStream
    {
        ...
        public MarshalAllocMemoryStream(long length)
        {
            if (length > int.MaxValue)
            {
                throw new InvalidOperationException($"Length cannot be longer than {int.MaxValue}");
            }

            m_Handle = Marshal.AllocHGlobal((int)length);
            unsafe
            {
                Initialize((byte*)m_Handle.ToPointer(), length, length, FileAccess.ReadWrite);
            }
